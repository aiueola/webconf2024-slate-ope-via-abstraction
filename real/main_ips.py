...
from src.estimators import InversePropensityScoring as IPS
from src.estimators import PseudoInverse as PI
from src.estimators import MarginalizedIPS as MIPS
from src.estimators import LatentInversePropensityScoring as LIPS
...
def evaluate_estimators(
    ...
    **kwargs,
    ...
    pi = PI(estimator_name="PI")
    mips = MIPS(estimator_name="MIPS")
    lips = LIPS(estimator_name="LIPS (Ours)")
    ope_estimators = [dm, ips, pi, mips, lips]
    # models
    ...
    baseline_estimation_dict = ope.estimate_policy_value_with_baseline_estimators(
        estimator_names=["DM", "IPS", "MIPS", "PI"],
...
def process(
    ...
    # aggregate result
    baseline_estimators_name = ["DM", "IPS", "PI", "MIPS", "on_policy"]
    ...
    for return_ in returns:
        ...
        for baseline_dict_, lips_dict_, target_value_ in zip(
            ...
            lips_performance["LIPS (Ours)"].append(estimation_[1])
    ...
    df["IPS"] = baseline_performance["IPS"]
    df["PI"] = baseline_performance["PI"]
    df["MIPS"] = baseline_performance["MIPS"]
    df["LIPS (Ours)"] = lips_performance["LIPS (Ours)"]
    df["best beta"] = lips_performance["best beta"]

    if report_results_for_all_beta:
        for beta in betas:
            df[f"LIPS ({beta})"] = lips_performance[beta]

    df["on_policy"] = baseline_performance["on_policy"]
    df["target_value"] = target_values
    df["random_state"] = random_states

    path_ = Path(log_dir + f"/logs/{dataset}/{reward_function_type}")
    path_.mkdir(exist_ok=True, parents=True)
    ...
    aggdf = (
        df[["DM", "IPS", "PI", "MIPS", "LIPS (Ours)", "on_policy", "target_value"]]
